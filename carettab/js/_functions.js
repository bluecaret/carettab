...
function tDimSeconds(loaded) {
    if (sDimSeconds == 'on') {
        if(loaded != true) { chrome.storage.sync.set({'sDimSeconds':'on'}, function() { if (!chrome.runtime.lastError) { console.log("^CaretTab - SAVED sDimSeconds: on"); }}); }
        $('html').addClass('on-dimseconds');
        $('#set-dimseconds').attr('Checked','Checked');
    } else {
        if(loaded != true) { chrome.storage.sync.set({'sDimSeconds':'off'}, function() { if (!chrome.runtime.lastError) { console.log("^CaretTab - SAVED sDimSeconds: off"); }}); }
...
function tDimDelimiter(loaded) {
    if (sDimDelimiter == 'on') {
        if(loaded != true) { chrome.storage.sync.set({'sDimDelimiter':'on'}, function() { if (!chrome.runtime.lastError) { console.log("^CaretTab - SAVED sDimDelimiter: on"); }}); }
        $('html').addClass('on-dimdelimiter');
        $('#set-dimdelimiter').attr('Checked','Checked');
    } else {
        if(loaded != true) { chrome.storage.sync.set({'sDimDelimiter':'off'}, function() { if (!chrome.runtime.lastError) { console.log("^CaretTab - SAVED sDimDelimiter: off"); }}); }
...
function tBlinking(loaded) {
    if (sBlinking == 'on') {
        ...
        var delimiter = $('#time .delimiter');
        if (sDelimiter == 'on') { $(delimiter).css({'opacity':''}); }
    }
}

function tPrimaryClockTimezone(value, loaded) {
    if(loaded != true) { chrome.storage.sync.set({'sPrimaryClockTimezone':value}, function() { if (!chrome.runtime.lastError) { console.log("^CaretTab - SAVED sPrimaryClockTimezone: "+value); }}); }
...
function tDay(loaded) {
    if (sDay == 'on') {
        ...
        updateDate();
        if (sTabTitle == "tab-date" || sTabTitle == "tab-timedate" || sTabTitle == "tab-datetime") { updateTabTitle(); }
    }
}
...
function tWeek(loaded) {
    if (sWeek == 'on') {
        if(loaded != true) { chrome.storage.sync.set({'sWeek':'on'}, function() { if (!chrome.runtime.lastError) { console.log("^CaretTab - SAVED sWeek: on"); }}); }
        $('html').addClass('on-week');
        $('#set-week').attr('Checked','Checked');
        updateDate();
    } else {
        if(loaded != true) { chrome.storage.sync.set({'sWeek':'off'}, function() { if (!chrome.runtime.lastError) { console.log("^CaretTab - SAVED sWeek: off"); }}); }
...
function tForeground(value, loaded) {
    ...
    $('#carettab a').css({'color':value});
    $('#carettab #search').css({'border-color':value, 'color':value});
    $('#carettab #search input').css({'border-color':value, 'color':value});
    $('#carettab #search button').css({'border-color':value, });
    $('#carettab #search button svg').css({'fill':value});
    $('#settings-toggle .st0').css({'fill':value});
    $('#carettab #analog circle').css({'fill':value});
    $('#carettab #analog ellipse').css({'fill':value});
    $('#carettab #analog .analog-face div').css({'background':value});
}
...
function tScale(loaded) {
    if(loaded != true) { chrome.storage.sync.set({'sScale':sScale}, function() { if (!chrome.runtime.lastError) { console.log("^CaretTab - SAVED sScale: "+sScale); }}); }
...
function tDimBrackets(loaded) {
    if (sDimBrackets == 'on') {
        if(loaded != true) { chrome.storage.sync.set({'sDimBrackets':'on'}, function() { if (!chrome.runtime.lastError) { console.log("^CaretTab - SAVED sDimBrackets: on"); }}); }
        $('html').addClass('on-dimbrackets');
        $('#set-dimbrackets').attr('Checked','Checked');
    } else {
        if(loaded != true) { chrome.storage.sync.set({'sDimBrackets':'off'}, function() { if (!chrome.runtime.lastError) { console.log("^CaretTab - SAVED sDimBrackets: off"); }}); }
...
function tBracketStyle(loaded) {
    if(loaded != true) { chrome.storage.sync.set({'sBracketStyle':sBracketStyle}, function() { if (!chrome.runtime.lastError) { console.log("^CaretTab - SAVED sBracketStyle: "+sBracketStyle); }}); }
...
function tDateDelimiter(loaded) {
    if(loaded != true) { chrome.storage.sync.set({'sDateDelimiter':sDateDelimiter}, function() { if (!chrome.runtime.lastError) { console.log("^CaretTab - SAVED sDateDelimiter: "+sDateDelimiter); }}); }
    updateDate();
...
function tWeekLabel(loaded) {
    if(loaded != true) { chrome.storage.sync.set({'sWeekLabel':sWeekLabel}, function() { if (!chrome.runtime.lastError) { console.log("^CaretTab - SAVED sWeekLabel: "+sWeekLabel); }}); }
    updateDate();
...
function tFont(loaded) {
    if(loaded != true) { chrome.storage.sync.set({'sFont':sFont}, function() { if (!chrome.runtime.lastError) { console.log("^CaretTab - SAVED sFont: "+sFont); }}); }
...
function tHideSettingsToggle(loaded) {
    if (sHideSettingsToggle == 'on') {
        if(loaded != true) { chrome.storage.sync.set({'sHideSettingsToggle':'on'}, function() { if (!chrome.runtime.lastError) { console.log("^CaretTab - SAVED sHideSettingsToggle: on"); }}); }
        $('html').removeClass('on-settings-toggle');
        $('#set-hide-settings-toggle').attr('Checked','Checked');
    } else {
        if(loaded != true) { chrome.storage.sync.set({'sHideSettingsToggle':'off'}, function() { if (!chrome.runtime.lastError) { console.log("^CaretTab - SAVED sHideSettingsToggle: off"); }}); }
...
function tAnalog(loaded) {
    if (sAnalog == 'on') {
        if(loaded != true) { chrome.storage.sync.set({'sAnalog':'on'}, function() { if (!chrome.runtime.lastError) { console.log("^CaretTab - SAVED sAnalog: on"); }}); }
        $('#set-analog').removeAttr('Checked');
        $('html').addClass('on-analog');
        updateAnalog();
    } else {
        if(loaded != true) { chrome.storage.sync.set({'sAnalog':'off'}, function() { if (!chrome.runtime.lastError) { console.log("^CaretTab - SAVED sAnalog: off"); }}); }
