export const languages = [
  { id: 'en-US', label: 'English', name: 'English (United States)' },
  { id: 'zh-CN', label: '简体中文（中国大陆）', name: 'Simplified Chinese (China Mainland)' },
  { id: 'zh-TW', label: '繁体中文（台湾）', name: 'Traditional Chinese (Taiwan)' },
  { id: 'fr-FR', label: 'Français', name: 'French (France)' },
  { id: 'de-DE', label: 'Deutsch', name: 'German (Germany)' },
  { id: 'he-IL', label: 'עברית', name: 'Hebrew (Israel)' },
  { id: 'it-IT', label: 'Italiano (Italia)', name: 'Italian (Italy)' },
  { id: 'ja-JP', label: '日本語', name: 'Japanese (Japan)' },
  { id: 'ko-KR', label: '한국어', name: 'Korean (Korea)' },
  { id: 'pt-BR', label: 'Portugues (brasil)', name: 'Portuguese (Brazil)' },
  { id: 'pt-PT', label: 'Portugues (portugal)', name: 'Portuguese (Portugal)' },
  { id: 'ru-RU', label: 'Pусский', name: 'Russian (Russia)' },
  { id: 'es-MX', label: 'Español', name: 'Spanish (Mexico)' },
  { id: 'sv-SE', label: 'Svenska (Sverige)', name: 'Swedish (Sweden)' },
  { id: 'uk-UA', label: 'Українська', name: 'Ukrainian (Ukraine)' },
  { id: 'ur-PK', label: 'اردو (پاکستان)', name: 'Urdu (Pakistan)' },
  { id: 'vi-VN', label: 'Tiếng Việt', name: 'Vietnamese (Vietnam)' }
];

export const span = [
  { id: 10, label: '25%', css: '25%' },
  { id: 20, label: '50%', css: '50%' },
  { id: 30, label: '75%', css: '75%' },
  { id: 40, label: '100%', css: '100%' }
];

export const title = [
  { id: 10, label: '"New Tab"', i18n: 'list.title.newTab' },
  { id: 20, label: 'Time', i18n: 'list.title.time' },
  { id: 30, label: 'Date', i18n: 'list.title.date' },
  { id: 40, label: 'Time, then date', i18n: 'list.title.timeDate' },
  { id: 50, label: 'Date, then time', i18n: 'list.title.dateTime' },
  { id: 60, label: 'Custom text', i18n: 'list.title.custom' }
];

export const fontList = [
  { id: 0, label: 'USE CUSTOM FONT', family: 'Arial', weight: 400, link: null, premium: false },
  { id: 10, label: 'Anton', family: 'Anton', weight: 400, link: null, premium: false },
  { id: 20, label: 'Buda', family: 'Buda', weight: 300, link: null, premium: false },
  { id: 30, label: 'Cabin Sketch', family: 'Cabin Sketch', weight: 400, link: null, premium: false },
  { id: 40, label: 'Chelsea Market', family: 'Chelsea Market', weight: 400, link: null, premium: false },
  { id: 50, label: 'Comfortaa', family: 'Comfortaa', weight: 400, link: null, premium: false },
  { id: 51, label: 'Comfortaa Light', family: 'Comfortaa', weight: 300, link: null, premium: false },
  { id: 52, label: 'Comfortaa Bold', family: 'Comfortaa', weight: 700, link: null, premium: false },
  { id: 60, label: 'Dancing Script', family: 'Dancing Script', weight: 400, link: null, premium: false },
  { id: 70, label: 'Digital 7 Mono (by Style-7)', family: 'Digital-7 Mono', weight: 400, link: null, premium: false },
  { id: 80, label: 'EB Garamond', family: 'EB Garamond', weight: 400, link: null, premium: false },
  { id: 81, label: 'EB Garamond Bold', family: 'EB Garamond', weight: 700, link: null, premium: false },
  { id: 90, label: 'Exo 2', family: 'Exo 2', weight: 400, link: null, premium: false },
  { id: 91, label: 'Exo 2 Extra Light', family: 'Exo 2', weight: 200, link: null, premium: false },
  { id: 92, label: 'Exo 2 Bold', family: 'Exo 2', weight: 700, link: null, premium: false },
  { id: 100, label: 'Inconsolata', family: 'Inconsolata', weight: 400, link: null, premium: false },
  { id: 101, label: 'Inconsolata Bold', family: 'Inconsolata', weight: 700, link: null, premium: false },
  { id: 110, label: 'Lobster', family: 'Lobster', weight: 400, link: null, premium: false },
  { id: 120, label: 'Lobster Two', family: 'Lobster Two', weight: 400, link: null, premium: false },
  { id: 130, label: 'Monoton', family: 'Monoton', weight: 400, link: null, premium: false },
  { id: 140, label: 'Nanum Pen Script', family: 'Nanum Pen Script', link: null, weight: 400, premium: false },
  { id: 150, label: 'Nixie One', family: 'Nixie One', weight: 400, link: null, premium: false },
  { id: 160, label: 'Open Sans', family: 'Open Sans', weight: 400, link: null, premium: false },
  { id: 161, label: 'Open Sans Light', family: 'Open Sans', weight: 300, link: null, premium: false },
  { id: 162, label: 'Open Sans Bold', family: 'Open Sans', weight: 700, link: null, premium: false },
  { id: 170, label: 'Overlock SC', family: 'Overlock SC', weight: 400, link: null, premium: false },
  { id: 180, label: 'Quicksand', family: 'Quicksand', weight: 400, link: null, premium: false },
  { id: 181, label: 'Quicksand Light', family: 'Quicksand', weight: 300, link: null, premium: false },
  { id: 182, label: 'Quicksand Bold', family: 'Quicksand', weight: 700, link: null, premium: false },
  { id: 190, label: 'Special Elite', family: 'Special Elite', weight: 400, link: null, premium: false },
  { id: 'abyssinica-sil', label: 'Abyssinica SIL', family: 'Abyssinica SIL', weight: 400, link: 'Abyssinica+SIL', premium: true },
  { id: 'alkalami', label: 'Alkalami', family: 'Alkalami', weight: 400, link: 'Alkalami', premium: true },
  { id: 'bebas-neue', label: 'Bebas Neue', family: 'Bebas Neue', weight: 400, link: 'Bebas+Neue', premium: true },
  { id: 'caveat', label: 'Caveat', family: 'Caveat', weight: 400, link: 'Caveat', premium: true },
  { id: 'gemunu-libre-light', label: 'Gemunu Libre Light', family: 'Gemunu Libre', weight: 300, link: 'Gemunu+Libre:wght@300', premium: true },
  { id: 'gemunu-libre', label: 'Gemunu Libre', family: 'Gemunu Libre', weight: 400, link: 'Gemunu+Libre:wght@400', premium: true },
  { id: 'josefin-sans-light', label: 'Josefin Sans Light', family: 'Josefin Sans', weight: 300, link: 'Josefin+Sans:wght@300', premium: true },
  { id: 'josefin-sans', label: 'Josefin Sans', family: 'Josefin Sans', weight: 400, link: 'Josefin+Sans:wght@400', premium: true },
  { id: 'merriweather-light', label: 'Merriweather Light', family: 'Merriweather', weight: 300, link: 'Merriweather:wght@300', premium: true },
  { id: 'merriweather', label: 'Merriweather', family: 'Merriweather', weight: 400, link: 'Merriweather:wght@400', premium: true },
  { id: 'montserrat-light', label: 'Montserrat Light', family: 'Montserrat', weight: 300, link: 'Montserrat:wght@300', premium: true },
  { id: 'montserrat', label: 'Montserrat', family: 'Montserrat', weight: 400, link: 'Montserrat:wght@400', premium: true },
  { id: 'mukta-light', label: 'Mukta Light', family: 'Mukta', weight: 300, link: 'Mukta:wght@300', premium: true },
  { id: 'mukta', label: 'Mukta', family: 'Mukta', weight: 400, link: 'Mukta:wght@400', premium: true },
  { id: 'oswald-light', label: 'Oswald Light', family: 'Oswald', weight: 300, link: 'Oswald:wght@300', premium: true },
  { id: 'oswald', label: 'Oswald', family: 'Oswald', weight: 400, link: 'Oswald:wght@400', premium: true },
  { id: 'playfair-display', label: 'Playfair Display', family: 'Playfair Display', weight: 400, link: 'Playfair+Display', premium: true },
  { id: 'raleway-light', label: 'Raleway Light', family: 'Raleway', weight: 300, link: 'Raleway:wght@300', premium: true },
  { id: 'raleway', label: 'Raleway', family: 'Raleway', weight: 400, link: 'Raleway:wght@400', premium: true },
  { id: 'roboto-light', label: 'Roboto Light', family: 'Roboto', weight: 300, link: 'Roboto:wght@300', premium: true },
  { id: 'roboto', label: 'Roboto', family: 'Roboto', weight: 400, link: 'Roboto:wght@400', premium: true },
  { id: 'roboto-condensed-light', label: 'Roboto Condensed Light', family: 'Roboto Condensed', weight: 300, link: 'Roboto+Condensed:wght@300', premium: true },
  { id: 'roboto-condensed', label: 'Roboto Condensed', family: 'Roboto Condensed', weight: 400, link: 'Roboto+Condensed:wght@400', premium: true },
  { id: 'roboto-mono-light', label: 'Roboto Mono Light', family: 'Roboto Mono', weight: 300, link: 'Roboto+Mono:wght@300', premium: true },
  { id: 'roboto-mono', label: 'Roboto Mono', family: 'Roboto Mono', weight: 400, link: 'Roboto+Mono:wght@400', premium: true },
  { id: 'roboto-slab-light', label: 'Roboto Slab Light', family: 'Roboto Slab', weight: 300, link: 'Roboto+Slab:wght@300', premium: true },
  { id: 'roboto-slab', label: 'Roboto Slab', family: 'Roboto Slab', weight: 400, link: 'Roboto+Slab:wght@400', premium: true },
  { id: 'silkscreen', label: 'Silkscreen', family: 'Silkscreen', weight: 400, link: 'Silkscreen', premium: true },
  { id: 'source-sans-pro-light', label: 'Source Sans Pro Light', family: 'Source Sans Pro', weight: 300, link: 'Source+Sans+Pro:wght@300', premium: true },
  { id: 'source-sans-pro', label: 'Source Sans Pro', family: 'Source Sans Pro', weight: 400, link: 'Source+Sans+Pro:wght@400', premium: true },
  { id: 'source-sans-pro-semibold', label: 'Source Sans Pro Semi-bold', family: 'Source Sans Pro', weight: 600, link: 'Source+Sans+Pro:wght@600', premium: true },
  { id: 'ubuntu-light', label: 'Ubuntu Light', family: 'Ubuntu', weight: 300, link: 'Ubuntu:wght@300', premium: true },
  { id: 'ubuntu', label: 'Ubuntu', family: 'Ubuntu', weight: 400, link: 'Ubuntu:wght@400', premium: true },
]

export const customFontWeight = [
  { id: 10, label: 'Thin', weight: 100 },
  { id: 20, label: 'Extra Light', weight: 200 },
  { id: 30, label: 'Light', weight: 300 },
  { id: 40, label: 'Regular', weight: 400 },
  { id: 50, label: 'Medium', weight: 500 },
  { id: 60, label: 'Semi-Bold', weight: 600 },
  { id: 70, label: 'Bold', weight: 700 },
  { id: 80, label: 'Black', weight: 900 },
];

export const analogFaceStyles = [
  { id: 'none', label: 'No face' },
  { id: 'dash', label: 'Dash' },
  { id: 'dot', label: 'Dot' },
  { id: 'romanDashHour', label: 'Roman Dash Hour' },
  { id: 'romanDashQuarter', label: 'Roman Dash Quarter' },
  { id: 'arabicDashHour', label: 'Arabic Dash Hour' },
  { id: 'arabicDashQuarter', label: 'Arabic Dash Quarter' },
  { id: 'swiss', label: 'Swiss' },
];

export const analogHandStyles = [
  { id: 'minimalThin', label: 'Minimal Thin' },
  { id: 'minimalMedium', label: 'Minimal Medium' },
  { id: 'minimalThick', label: 'Minimal Thick' },
  { id: 'pointed', label: 'Pointed' },
  { id: 'modern', label: 'Modern' },
  { id: 'modern2', label: 'Modern 2' },
  { id: 'swiss', label: 'Swiss' },
];

export const dateFormat = [
  { id: 10, label: 'Little-Endian (31 Jan. 2000)' },
  { id: 20, label: 'Middle-Endian (Jan. 31, 2000)' },
  { id: 30, label: 'Big-Endian (2000 Jan. 31)' }
];

export const searchEngines = [
  { id: 10, label: 'Google', url: 'https://www.google.com/search', param: 'q' },
  { id: 20, label: 'Bing', url: 'https://www.bing.com/search', param: 'q' },
  { id: 30, label: 'Baidu', url: 'https://www.baidu.com/s', param: 'wd' },
  { id: 40, label: 'Yahoo', url: 'https://search.yahoo.com/search', param: 'p' },
  { id: 50, label: 'Ask.com', url: 'https://www.ask.com/web', param: 'q' },
  { id: 60, label: 'DuckDuckGo', url: 'https://www.duckduckgo.com/', param: 'q' },
  { id: 70, label: 'Wolframalpha', url: 'https://www.wolframalpha.com/input/', param: 'i' },
  { id: 80, label: 'GitHub', url: 'https://www.github.com/search', param: 'q' },
  { id: 90, label: 'Yandex', url: 'https://yandex.com/search/', param: 'text' }
];

export const bgBlend = [
  { id: 10, label: 'Normal', mode: 'normal' },
  { id: 20, label: 'Color', mode: 'color' },
  { id: 30, label: 'Color burn', mode: 'color-burn' },
  { id: 40, label: 'Color dodge', mode: 'color-dodge' },
  { id: 50, label: 'Darken', mode: 'darken' },
  { id: 60, label: 'Lighten', mode: 'lighten' },
  { id: 70, label: 'Difference', mode: 'difference' },
  { id: 80, label: 'Exclusion', mode: 'exclusion' },
  { id: 90, label: 'Hard light', mode: 'hard-light' },
  { id: 100, label: 'Soft light', mode: 'soft-light' },
  { id: 110, label: 'Hue', mode: 'hue' },
  { id: 120, label: 'Saturation', mode: 'saturation' },
  { id: 130, label: 'Luminosity', mode: 'luminosity' },
  { id: 140, label: 'Multiply', mode: 'multiply' },
  { id: 150, label: 'Screen', mode: 'screen' },
  { id: 160, label: 'Overlay', mode: 'overlay' },
];

export const bgSize = [
  { id: 10, label: 'Repeat', size: 'auto' },
  { id: 20, label: 'Cover', size: 'cover' },
  { id: 30, label: 'Contain', size: 'contain' },
  { id: 40, label: 'Scale', size: 'scale' },
  { id: 50, label: 'Scale and repeat', size: 'scaleRepeat' },
];

export const colors = [
  { id: 100, bg: '#FFFFFF', fg: '#000000' },
  { id: 200, bg: '#FFFFFF', fg: '#7AC943' },
  { id: 300, bg: '#FFFFFF', fg: '#00A99D' },
  { id: 400, bg: '#FFFFFF', fg: '#3FA9F5' },
  { id: 500, bg: '#FFFFFF', fg: '#93278F' },
  { id: 600, bg: '#FFFFFF', fg: '#FF7BAC' },
  { id: 700, bg: '#FFFFFF', fg: '#FF1D25' },
  { id: 800, bg: '#FFFFFF', fg: '#FF931E' },
  { id: 900, bg: '#000000', fg: '#FFFFFF' },
  { id: 1000, bg: '#000000', fg: '#7AC943' },
  { id: 1100, bg: '#000000', fg: '#00A99D' },
  { id: 1200, bg: '#000000', fg: '#3FA9F5' },
  { id: 1300, bg: '#000000', fg: '#93278F' },
  { id: 1400, bg: '#000000', fg: '#FF7BAC' },
  { id: 1500, bg: '#000000', fg: '#FF1D25' },
  { id: 1600, bg: '#000000', fg: '#FF931E' },
  { id: 1700, bg: '#E6E6E6', fg: '#333333' },
  { id: 1800, bg: '#7AC943', fg: '#000000' },
  { id: 1900, bg: '#00A99D', fg: '#000000' },
  { id: 2000, bg: '#3FA9F5', fg: '#000000' },
  { id: 2100, bg: '#93278F', fg: '#000000' },
  { id: 2200, bg: '#FF7BAC', fg: '#000000' },
  { id: 2300, bg: '#FF1D25', fg: '#000000' },
  { id: 2400, bg: '#FF931E', fg: '#000000' },
  { id: 2500, bg: '#333333', fg: '#E6E6E6' },
  { id: 2600, bg: '#7AC943', fg: '#FFFFFF' },
  { id: 2700, bg: '#00A99D', fg: '#FFFFFF' },
  { id: 2800, bg: '#3FA9F5', fg: '#FFFFFF' },
  { id: 2900, bg: '#93278F', fg: '#FFFFFF' },
  { id: 3000, bg: '#FF7BAC', fg: '#FFFFFF' },
  { id: 3100, bg: '#FF1D25', fg: '#FFFFFF' },
  { id: 3200, bg: '#FF931E', fg: '#FFFFFF' },
  { id: 3300, bg: '#BDCCD4', fg: '#534741' },
  { id: 3400, bg: '#E2EDC1', fg: '#00A055' },
  { id: 3500, bg: '#C9F2EE', fg: '#0000FF' },
  { id: 3600, bg: '#C8EAF5', fg: '#00AEEF' },
  { id: 3700, bg: '#D8D6EC', fg: '#702C8D' },
  { id: 3800, bg: '#FCE3EE', fg: '#F190AC' },
  { id: 3900, bg: '#EEDFAC', fg: '#534741' },
  { id: 4000, bg: '#FFFAC0', fg: '#F4783B' },
  { id: 4100, bg: '#E1E9F2', fg: '#004080' },
  { id: 4200, bg: '#EAF3E0', fg: '#008080' },
  { id: 4300, bg: '#004040', fg: '#FFFFFF' },
  { id: 4400, bg: '#0000FF', fg: '#FFFFFF' },
  { id: 4500, bg: '#93278F', fg: '#FF931E' },
  { id: 4600, bg: '#FAC8D3', fg: '#895881' },
  { id: 4700, bg: '#C1172C', fg: '#FFF200' },
  { id: 4800, bg: '#FF8040', fg: '#000000' },
];

// Patterns from Transparent Textures - Built and maintained by @mikehearn.
// https://www.transparenttextures.com/
export const patterns = [
  { id: 200, pattern: 'checkered-pattern.png' },
  { id: 300, pattern: 'escheresque-dark.png' },
  { id: 400, pattern: '45-degree-fabric-dark.png' },
  { id: 600, pattern: 'flowers.png' },
  { id: 700, pattern: '60-lines.png' },
  { id: 800, pattern: 'clean-gray-paper.png' },
  { id: 1000, pattern: 'always-grey.png' },
  { id: 1100, pattern: 'climpek.png' },
  { id: 1200, pattern: 'gplay.png' },
  { id: 1300, pattern: 'arabesque.png' },
  { id: 1400, pattern: 'concrete-wall.png' },
  { id: 1600, pattern: 'arches.png' },
  { id: 1700, pattern: 'connected.png' },
  { id: 1900, pattern: 'argyle.png' },
  { id: 2000, pattern: 'crisp-paper-ruffles.png' },
  { id: 2100, pattern: 'inspiration-geometry.png' },
  { id: 2300, pattern: 'crissxcross.png' },
  { id: 2400, pattern: 'lined-paper-2.png' },
  { id: 2500, pattern: 'back-pattern.png' },
  { id: 2600, pattern: 'cubes.png' },
  { id: 2700, pattern: 'mirrored-squares.png' },
  { id: 2800, pattern: 'batthern.png' },
  { id: 2900, pattern: 'dark-brick-wall.png' },
  { id: 3000, pattern: 'ps-neutral.png' },
  { id: 3100, pattern: 'binding-dark.png' },
  { id: 3300, pattern: 'purty-wood.png' },
  { id: 3500, pattern: 'dark-geometric.png' },
  { id: 3600, pattern: 'retina-wood.png' },
  { id: 3700, pattern: 'black-thread-light.png' },
  { id: 3800, pattern: 'dark-leather.png' },
  { id: 3900, pattern: 'shattered.png' },
  { id: 4000, pattern: 'brick-wall.png' },
  { id: 4100, pattern: 'dark-mosaic.png' },
  { id: 4200, pattern: 'skulls.png' },
  { id: 4300, pattern: 'brick-wall-dark.png' },
  { id: 4400, pattern: 'diagmonds.png' },
  { id: 4500, pattern: 'tileable-wood.png' },
  { id: 4600, pattern: 'bright-squares.png' },
  { id: 4700, pattern: 'diagmonds-light.png' },
  { id: 4800, pattern: 'tree-bark.png' },
  { id: 4900, pattern: 'candyhole.png' },
  { id: 5100, pattern: 'use-your-illusion.png' },
  { id: 5300, pattern: 'diamond-upholstery.png' },
  { id: 5400, pattern: 'white-diamond.png' },
  { id: 5500, pattern: 'cartographer.png' },
  { id: 5600, pattern: 'escheresque.png' },
  { id: 5700, pattern: 'xv.png' }
];
