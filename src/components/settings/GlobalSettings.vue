...
const openPatterns = ref(false)
const openUnsplash = ref(false)
...
const handlePatternExit = () => {
  openPatterns.value = false
}

const handleUnsplashExit = () => {
  openUnsplash.value = false
}
...
<template>
  <div v-if="!openPatterns && !openUnsplash" class="page">
    <h2 class="title">
      <button
        class="back"
        :aria-label="$t('options.dashboard.backToDashboard')"
      ...
      <div class="block">
        <div class="group fill">
          ...
          <div v-if="store.config.global.it && store.config.global.it !== 'none'" class="group compact">
            <button v-if="store.config.global.it === 'pattern'" class="btn" type="button" @click="openPatterns = true">
              Select pattern
            </button>
            <button
          ...
          <div v-if="!store.config.global.it || store.config.global.it === 'none'" class="group compact">
            ...
            <div class="btnGroup">
              ...
              <button class="btn" type="button" @click="openPatterns = true">Pattern</button>
              <button class="btn" type="button" @click="openUnsplash = true"><PremiumLabel />Unsplash.com</button>
            </div>
          </div>
        </div>
    ...
    <h3 class="subtitle">Default widget styles</h3>
    <div class="blockContainer">
      <WidgetFontField
  ...
  <PatternsList v-if="openPatterns" @back="handlePatternExit"></PatternsList>
  <UnsplashList v-if="openUnsplash" @back="handleUnsplashExit"></UnsplashList>
</template>
