...
const onPrompt = async () => {
  ...
  prompt.value = ''
  try {
    const configuration = new Configuration({ apiKey: tool.apiKey })
    ...
    if (error.response) {
      errorMessage.value = `
        ...
        ${error.response.data.error.type ? ' - ' + error.response.data.error.type : ''}
        ${error.response.data.error.message ? ' - ' + error.response.data.error.message : ''}
      `
      console.warn(error.response.status, error.response.data)
    } else {
      errorMessage.value = `ChatGPT responded with an error: ${error.message}`
...
<template>
  <div ref="toolPanelEl" class="toolbarToolPanel panel">
    <div class="page">
      <h2 class="title">
        AI Chat
        <button
...
.loading .block {
  ...
  .dot {
    ...
    opacity: 0;
    animation: dot 1.2s steps(2, jump-none) infinite;
    &:nth-child(2) {
